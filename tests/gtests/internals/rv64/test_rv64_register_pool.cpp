...
TEST(RegisterPoolPartition, SingleAllocAdvancesByOne) {
...
TEST(RegisterPoolPartition, PanelAllocAdvancesByCols) {
    register_pool_t::x_partition_t p({a0, a1, a2, a3});
    p.allocate(3);
    ASSERT_EQ(p.allocated(), 3);
}

TEST(RegisterPoolPartition, BlockAllocAdvancesByRowsTimesCols) {
    register_pool_t::x_partition_t p({a0, a1, a2, a3, a5, a6, a7, s0, s1, s2});
    p.allocate(2, 3);
    ASSERT_EQ(p.allocated(), 6);
}

TEST(RegisterPoolPartition, MixedAllocationsTrackCorrectly) {
    register_pool_t::x_partition_t p({a0, a1, a2, a3});
    p.allocate(); // 1
    p.allocate(2); // +2 = 3
    ASSERT_EQ(p.allocated(), 3);
...
TEST(RegisterPoolPartition, FailedSingleAlloc) {
...
TEST(RegisterPoolPartition, FailedMultiAlloc) {
...
struct preserve_restore_probe_t : public jit_generator_t {
    DECLARE_CPU_JIT_AUX_FUNCTIONS(preserve_restore_probe_t)

    preserve_restore_probe_t(std::vector<Reg> excl, int n_alloc)
        : jit_generator_t("preserve_restore_probe")
        , excl_(std::move(excl))
        , n_alloc_(n_alloc) {}
...
// stack offset.
TEST(RegisterPoolJIT, PreserveRestoreEmitsMatchedSdLdPairs) {
...
TEST(RegisterPoolJIT, PreserveRestoreIsNoOpWhenNoCalleeSavedAllocated) {
...
struct acc_blocks_inp_probe_t : public jit_generator_t {
    DECLARE_CPU_JIT_AUX_FUNCTIONS(acc_blocks_inp_probe_t)
    acc_blocks_inp_probe_t() : jit_generator_t("vector_split_acc_blocks_inp") {}
    int acc0_idx = -1;
    int inp0_idx = -1;
...
struct inp_blocks_acc_probe_t : public jit_generator_t {
    DECLARE_CPU_JIT_AUX_FUNCTIONS(inp_blocks_acc_probe_t)
    inp_blocks_acc_probe_t() : jit_generator_t("vector_split_inp_blocks_acc") {}
    int inp0_idx = -1;
    int acc1_idx = -1;
...
struct single_pool_probe_t : public jit_generator_t {
    DECLARE_CPU_JIT_AUX_FUNCTIONS(single_pool_probe_t)
    single_pool_probe_t() : jit_generator_t("vector_split_single_pool") {}
...
HANDLE_EXCEPTIONS_FOR_TEST(
        RegisterPoolVectorSplit, AccAllocationBlocksOverlappingInpCandidates) {
...
HANDLE_EXCEPTIONS_FOR_TEST(
        RegisterPoolVectorSplit, InpAllocationBlocksOverlappingAccCandidate) {
...
HANDLE_EXCEPTIONS_FOR_TEST(
        RegisterPoolVectorSplit, SinglePoolFallbackMatchesNewVector) {
